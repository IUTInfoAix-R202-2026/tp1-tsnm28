...
8. **`leSliderALesBonnesLimites`** : configurez le slider avec `new Slider(0.1, 5, 1)` (min, max, valeur initiale). Pour coller à la maquette, **affichez ses bornes** : `setShowTickLabels(true)`, `setShowTickMarks(true)`, `setMajorTickUnit(4.9)` et `setMinorTickCount(0)` (un pas égal à la plage ne marque que le min et le max). Limitez aussi sa largeur (`setMaxWidth(320)`) pour **rapprocher les bornes** au lieu de les laisser aux deux extrémités de la fenêtre.
